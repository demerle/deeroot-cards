...
namespace DeerootCards.Cards
{
    ...
    /// </summary>
    public class SimulacrumCard : CustomCard
    {
        ...
        public static void Init()
        {
            ...
            harmonyApplied = true;
            var harmony = new Harmony("com.deeroot.cards.simulacrum");
            harmony.PatchAll(typeof(SimulacrumFriendlyFirePatch));
            SimulacrumBot.RegisterRoundResetHooks();
        }
    ...
    public static class SimulacrumBot
    {
        ...
        public static void SpawnBot(Player master, Vector3 pos)
        {
            ...
            if (prefab == null)
            {
                UnityEngine.Debug.LogError("[DEER] Simulacrum: no PlayerAssigner.instance.playerPrefab to clone!");
                return;
            }
            ...
            if (bot == null)
            {
                UnityEngine.Debug.LogError("[DEER] Simulacrum: player prefab has no CharacterData!");
                return;
            }
        ...
        private static void ConfigureBot(int masterPlayerID, int botViewID, Vector3 spawnPos)
        {
            ...
            {
                UnityEngine.Debug.LogWarning($"[DEER] Simulacrum: bot view {botViewID} vanished before config");
                return;
            }
            ...
            if (!view.IsMine)
            {
                ...
                if (pm != null && pm.players.Contains(bot.player))
                {
                    ...
                    UnityEngine.Debug.Log("[DEER] Simulacrum: stripped remote clone from PlayerManager registry");
                }
            }
            ...
            if (view.IsMine)
            {
                ...
                var brain = bot.gameObject.AddComponent<SimulacrumBotBrain>();
            ...
            UnityEngine.Debug.Log($"[DEER] Simulacrum: bot configured (owner client: {view.IsMine}, master {masterPlayerID}, HP {bot.maxHealth})");
        }
        ...
        private static IEnumerator EnforcePosition(CharacterData bot, Vector3 spawnPos)
        {
            ...
            for (int frame = 0; frame < 3; frame++)
            {
                ...
                UnityEngine.Debug.Log($"[DEER] Simulacrum: pos frame {frame}: want {spawnPos} got {bot.transform.position}");
                yield return null;
            }
        }
    ...
    /// </summary>
    public class SimulacrumBotBrain : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (canSeeTarget && distanceToTarget < SimulacrumBot.ShootRange && untilNextShot <= 0f && api.CanShoot())
            {
                api.Attack();
                untilNextShot = Random.Range(SimulacrumBot.FireCooldownMin, SimulacrumBot.FireCooldownMax);
            }
        }
        ...
        private void UpdateData()
        {
            ...
            aimDir += Vector3.up * (distanceToTarget * SimulacrumBot.AimCompensation);
            aimDir.Normalize();

            moveDir = new Vector3(Mathf.Sign(aimDir.x) * (Mathf.Abs(aimDir.x) > 0.2f ? 1f : 0f), 0f, 0f);
            if (canSeeTarget)
            {
                if (distanceToTarget < SimulacrumBot.TooCloseRange)
                {
                    // back off
                    moveDir = new Vector3(-moveDir.x, 0f, 0f);
                }
                else if (distanceToTarget <= SimulacrumBot.KeepRange && data.ThereIsGroundBelow(transform.position, 10f))
                {
    ...
    /// </summary>
    public class SimulacrumRunner : MonoBehaviour
    {
        private static SimulacrumRunner runner;

        internal static SimulacrumRunner Get()
        {
            if (runner == null)
            {
                var go = new GameObject("DeerootSimulacrumRunner");
                UnityEngine.Object.DontDestroyOnLoad(go);
                runner = go.AddComponent<SimulacrumRunner>();
            }
            return runner;
        }
    ...
    ///   - ALly bullets (ownPlayer on the master's side) hitting a Simulacrum bot
    ///     are swallowed, so teammates can't wipe your army by accident,
    ///   - Simulacrum-bot bullets never damage their master's team (targeting
    ///     already avoids it; this covers stray shots).
    /// Explosion/AoE damage vs bots still lands (known stage-1 limitation).
    /// </summary>
    [HarmonyPatch]
    public class SimulacrumFriendlyFirePatch
    {
        ...
        static bool Prefix(ProjectileHit __instance, int viewID, bool wasBlocked)
        {
            ...
            if (SimulacrumBot.IsBot(victimData.player))
            {
                if (victimData.player.teamID == shooter.teamID)
                {
                    UnityEngine.Debug.Log("[DEER] Simulacrum: swallowed friendly hit on bot");
                    return false;
                }
            ...
            if (SimulacrumBot.IsBot(shooter) && victimData.player.teamID == shooter.teamID)
            {
                UnityEngine.Debug.Log("[DEER] Simulacrum: swallowed bot hit on own team");
                return false;
            }
            return true;
        }
