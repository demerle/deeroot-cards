...
- **First one-shot ability card**: `C` (P2: `V`) → meteor falls from the TOP OF THE SCREEN (horizontal match with cursor; vertical anchor = `cam.ScreenToWorldPoint(mouse.x, cam.pixelHeight).y + 0.5` on the caster — position travels in the RPC so all clients spawn identically) at fixed stats — damage 500, initial speed 25 (= 50% of Bullet_Base's default 50), Bullet_Base's built-in gravity (100) accelerates it like a real meteor, ~3x vanilla-bullet knockback (`BaseForce = 180` pre-scaled; `ProjectileHit.Start` multiplies force by `(damage/55)²`, so 500 damage ⇒ ×82.6 — direct high force would be a nuke, tune `BaseForce` not `damage`).
- **Size scales with damage (Full-Counter recipe applies)**: `MeteorProjectileInit` rescales the bullet for the fixed damage — `oldTrailSize = TrailSizeFromDamage(PrefabStockDamage=55)`, `newTrailSize = TrailSizeFromDamage(500)`, ratio-rescale `SetScaleFromSizeAndExtraSize` sprite scale, rewrite `RayCastTrail.size` (detection radius), `ScaleTrailFromDamage.Rescale()` for trail width (all helpers copied from FullCounterCard, once inside the `configured` guard so the double-assert can't double-rescale). Playtest gate: the `[DEER] Meteor rescaled` log size jump.
