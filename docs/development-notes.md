...
- Bullet amplification = mutate the public `ProjectileHit.damage` and `MoveTransform.velocity` fields in a `MoveTransform.Update` postfix (same chokepoint as Portal/Heart). `RayCastTrail.MoveRay()` is NOT needed when the bullet's position is never displaced (only velocity/damage change) — MoveRay is for teleport-style position jumps. **Damage bumps after spawn also need the Full Counter visual-rescale block** (sprite ratio-rescale via `SetScaleFromSizeAndExtraSize`, `trail.size` recompute, `ScaleTrailFromDamage.Rescale()` via `GetComponentInDisabled`) — applied to Amp Wall's boost, playtest pending.
- `MoveTransform.multiplier` is public but `simulationSpeed` is internal (default 1) — segment reconstruction `prev = pos - velocity * TimeHandler.deltaTime * multiplier` is exact enough with `simulationSpeed` ignored for vanilla projectiles (Portal pattern already does this).
- Line-segment vs wall-segment intersection (parametric t/u solve) is the swept crossing test for walls; add a small `u` margin so tip grazes count, and a point-radius fallback for bullets spawned point-blank inside the wall. Parallel shots (denominator ≈ 0) must return false — flying along the wall is not crossing it.
- Sprite.Create with `pixelsPerUnit = texWidth / desiredWorldLength` bakes the world-space size into the sprite — no transform.localScale scaling needed for procedural bars.
- Placement RPC pattern for position+direction state: `NetworkingManager.RPC(..., playerID, center, Vector2 along)` into a static `Dictionary<int, WallData>` — same registry shape as Portal; every client runs detection locally.
