...
- **First one-shot ability card**: `C` (P2: `V`) → meteor falls from the TOP OF THE SCREEN (horizontal match with cursor; vertical anchor = `cam.ScreenToWorldPoint(mouse.x, cam.pixelHeight).y + 0.5` on the caster — position travels in the RPC so all clients spawn identically) at fixed stats — damage 1200, initial speed 20 (= 40% of Bullet_Base's default 50), Bullet_Base's built-in gravity (100) accelerates it like a real meteor, knockback pre-scaled to land ~3x a vanilla 55-damage bullet (`BaseForce = 31`; `ProjectileHit.Start` multiplies force by `(damage/55)²` — 1200 ⇒ ×476, so tune `BaseForce` alongside `BaseDamage` to keep total ≈ 15k), ~14x stock size via the rescale below.
- **Size scales with damage (Full-Counter recipe applies)**: `MeteorProjectileInit` rescales the bullet for the fixed damage — `oldTrailSize = TrailSizeFromDamage(PrefabStockDamage=55)`, `newTrailSize = TrailSizeFromDamage(BaseDamage)`, ratio-rescale `SetScaleFromSizeAndExtraSize` sprite scale, rewrite `RayCastTrail.size` (detection radius), `ScaleTrailFromDamage.Rescale()` for trail width (all helpers copied from FullCounterCard, once inside the `configured` guard so the double-assert can't double-rescale). Playtest gate: the `[DEER] Meteor rescaled` log size jump.
...
- **PITFALL (found in a debug round — the "ricochet" bug): hand-bolted bullets MUST also wire `ProjectileHit.spawnedAttack`** — the private field (ProjectileHit.cs:67) is only ever assigned by `Start` when the `SpawnedAttack` component already exists (vanilla `BulletInit` adds it BEFORE Start). `RPCA_DoHit` reads `spawnedAttack.spawner.data` on EVERY physics-object impact (314/323) — with it null the impact path NREs before `DestroyMe()` ⇒ the meteor never dies and re-hits the map every frame (visual bounce). Fix (`MeteorProjectileInit.Configure`): add/resolve `SpawnedAttack`, reflect it into the private `spawnedAttack` field, and mirror vanilla attribution — `spawner = caster`, `ownPlayer = caster`, `attackID` from the caster's gun, `team = PlayerSkinBank.GetPlayerSkinColors(pid)` + `SetTeamColor.TeamColorThis`. Never spawn a bullet via `PhotonNetwork.Instantiate` without this.
- Stats card lines: damage 1200, projectile speed 40%, uses 1. `allowMultiple = false` (per-player unique, per ability-card conventions) and `"Meteor"` added to `DoubleCard.AbilityCardNames` (compensates with Ability Up).
