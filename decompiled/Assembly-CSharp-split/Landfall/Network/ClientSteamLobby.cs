...
public class ClientSteamLobby
{
	...
	public bool IsMaster => SteamMatchmaking.GetLobbyOwner(CurrentLobby) == SteamUser.GetSteamID();
	...
	{
		AddOnLobbyCreatedAction(delegate
		{
	...
	public void GetFriendsPlayingROUNDS(Action<List<CSteamID>> a)
	{
		...
		for (int j = 0; j < friendCount; j++)
		{
			...
			if (SteamFriends.GetFriendGamePlayed(friendByIndex, out var pFriendGameInfo) && pFriendGameInfo.m_gameID.AppID() == ROUNDS_APPID && !list.Contains(friendByIndex))
			{
	...
	public void HideLobby()
	{
		...
		{
			SteamMatchmaking.SetLobbyJoinable(CurrentLobby, bLobbyJoinable: false);
		}
	}
	...
	public void OpenLobby()
	{
		if (m_IsActive && IsMaster)
		{
			SteamMatchmaking.SetLobbyJoinable(CurrentLobby, bLobbyJoinable: true);
		}
	}
	...
	{
		_ = param.m_rgfChatMemberStateChange;
		if (new CSteamID(param.m_ulSteamIDLobby) == CurrentLobby)
		{
			UpdateLobbyMembers();
		}
