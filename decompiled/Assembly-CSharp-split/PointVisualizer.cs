...
public class PointVisualizer : MonoBehaviour
{
	...
	public IEnumerator DoWinSequence(int orangePoints, int bluePoints, int orangeRounds, int blueRounds, bool orangeWinner)
	{
		...
		while (c < timeToScale)
		{
			if (orangeWinner)
			{
				orangeBallRT.sizeDelta = Vector2.LerpUnclamped(orangeBallRT.sizeDelta, Vector2.one * ballSmallSize, scaleCurve.Evaluate(c / timeToScale));
			}
			else
			{
				blueBallRT.sizeDelta = Vector2.LerpUnclamped(blueBallRT.sizeDelta, Vector2.one * ballSmallSize, scaleCurve.Evaluate(c / timeToScale));
			}
		...
		while (c < timeToMove)
		{
			if (orangeWinner)
			{
				orangeBall.position = Vector3.LerpUnclamped(orangeBall.position, UIHandler.instance.roundCounterSmall.GetPointPos(0), scaleCurve.Evaluate(c / timeToMove));
			}
			else
			{
				blueBall.position = Vector3.LerpUnclamped(blueBall.position, UIHandler.instance.roundCounterSmall.GetPointPos(1), scaleCurve.Evaluate(c / timeToMove));
			}
			c += Time.unscaledDeltaTime;
			yield return null;
		}
		...
		while (c < timeToMove)
		{
			if (!orangeWinner)
			{
				orangeBall.position = Vector3.LerpUnclamped(orangeBall.position, CardChoiceVisuals.instance.transform.position, scaleCurve.Evaluate(c / timeToMove));
			}
			else
			{
				blueBall.position = Vector3.LerpUnclamped(blueBall.position, CardChoiceVisuals.instance.transform.position, scaleCurve.Evaluate(c / timeToMove));
			}
			c += Time.unscaledDeltaTime;
			yield return null;
		}
		...
		while (c < timeToScale)
		{
			if (!orangeWinner)
			{
				orangeBallRT.sizeDelta = Vector2.LerpUnclamped(orangeBallRT.sizeDelta, Vector2.one * bigBallScale, scaleCurve.Evaluate(c / timeToScale));
			}
			else
			{
				blueBallRT.sizeDelta = Vector2.LerpUnclamped(blueBallRT.sizeDelta, Vector2.one * bigBallScale, scaleCurve.Evaluate(c / timeToScale));
			}
			c += Time.unscaledDeltaTime;
			yield return null;
		}
