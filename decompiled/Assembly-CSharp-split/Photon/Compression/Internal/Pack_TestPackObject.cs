...
public static class Pack_TestPackObject
{
	...
	public static void Initialize()
	{
		if (!initialized)
		{
			isInitializing = true;
			int num = 0;
			PackObjectAttribute packObjectAttribute = typeof(TestPackObject).GetCustomAttributes(typeof(PackObjectAttribute), inherit: false)[0] as PackObjectAttribute;
			DefaultKeyRate defaultKeyRate = packObjectAttribute.defaultKeyRate;
			FastBitMask128 defaultReadyMask = new FastBitMask128(2);
			int num2 = 0;
			SyncHalfFloatAttribute syncHalfFloatAttribute = typeof(TestPackObject).GetField("rotation").GetCustomAttributes(typeof(SyncVarBaseAttribute), inherit: false)[0] as SyncHalfFloatAttribute;
			...
			num += 16;
			num2++;
			SyncRangedIntAttribute syncRangedIntAttribute = typeof(TestPackObject).GetField("intoroboto").GetCustomAttributes(typeof(SyncVarBaseAttribute), inherit: false)[0] as SyncRangedIntAttribute;
