...
public class SyncState : SyncObject<SyncState.Frame>, IMountable, IOnCaptureState, IOnNetSerialize, IOnSnapshot, IReadyable, IOnNetObjReady, IUseKeyframes
{
	...
	public override int ApplyOrder => 5;
	...
	public Rigidbody Rb => netObj.Rb;

	public Rigidbody2D Rb2d => netObj.Rb2D;

	public override bool AllowReconstructionOfEmpty => false;
	...
	{
		ChangeState(new StateChangeInfo(initialState, base.transform.parent ? base.transform.parent.GetComponent<Mount>() : null, force: true));
		base.OnStart();
		respawnStateInfo = new StateChangeInfo(respawnState, base.transform.parent ? base.transform.parent.GetComponent<Mount>() : null, base.transform.localPosition, base.transform.localRotation, null, force: true);
	...
	public virtual void OnNetObjReadyChange(bool ready)
	{
		...
		if (!base.IsMine || iSpawnController == null || iSpawnController.AllowNetObjectReadyCallback(ready))
		{
			if (ready)
			{
				ChangeState(new StateChangeInfo(readyState, currentMount, force: true));
			}
			else
			{
				ChangeState(new StateChangeInfo(unreadyState, currentMount, force: true));
			}
		}
	}
	...
	public void SoftMount(Mount attachTo)
	{
		...
		ChangeState(new StateChangeInfo(itemState, attachTo, force: false));
	}
	...
	public void HardMount(Mount mountTo)
	{
		...
		ChangeState(new StateChangeInfo(itemState, mountTo, force: false));
	}
	...
	public void Despawn(bool immediate)
	{
		if (immediate)
		{
			ChangeState(new StateChangeInfo(ObjState.Despawned, null, force: true));
		}
		else
		{
			stateChangeQueue.Enqueue(new StateChangeInfo(ObjState.Despawned, null, force: true));
		}
	}
	...
	{
		stateChangeQueue.Clear();
		ChangeState(new StateChangeInfo((ObjState)49, null, force: true));
	}
	...
	{
		stateChangeQueue.Enqueue(new StateChangeInfo((ObjState)49, null, position, rotation, velocity, force: false));
	}

	public void ThrowLocal(Transform origin, Vector3 offset, Vector3 velocity)
	{
		stateChangeQueue.Enqueue(new StateChangeInfo((ObjState)49, null, origin.TransformPoint(offset), origin.TransformPoint(velocity), force: false));
	}
	...
	protected virtual void ChangeState(StateChangeInfo stateChangeInfo)
	{
		...
		bool flag;
		if (autoReset && state == ObjState.Despawned && objState != ObjState.Despawned && (objState & ObjState.Anchored) == 0)
		{
	...
	private void ChangeOwnerToParentMountsOwner()
	{
		...
		else
		{
			...
			if ((bool)photonView)
			{
				int newOwnerId = photonView.Owner?.ActorNumber ?? 0;
				if (autoOwnerChange)
				{
	...
	protected virtual void InvalidMountHandler(ObjState newState, Mount newMount, bool force)
	{
		...
		ChangeState(new StateChangeInfo(ObjState.Visible, null, force: true));
	}
	...
	public virtual bool ChangeMount(int newMountId)
	{
		...
		ChangeState(new StateChangeInfo(currentState.state, mount, force: false));
		return true;
	}
