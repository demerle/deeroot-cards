...
public class SyncTransform : SyncObject<SyncTransform.Frame>, ISyncTransform, IOnSnapshot, IOnNetSerialize, IOnAuthorityChanged, IReadyable, IUseKeyframes, IDeltaFrameChangeDetect, IOnInterpolate, IOnCaptureState, IFlagTeleport
{
	...
	{
		PosCrusher = new ElementCrusher(TRSType.Position, enableTRSTypeSelector: false)
		{
			hideFieldName = true,
			XCrusher = new FloatCrusher(Axis.X, TRSType.Position, showEnableToggle: true)
			{
				BitsDeterminedBy = BitsDeterminedBy.HalfFloat,
				AccurateCenter = true
			},
			YCrusher = new FloatCrusher(Axis.Y, TRSType.Position, showEnableToggle: true)
			{
				BitsDeterminedBy = BitsDeterminedBy.HalfFloat,
				AccurateCenter = true
			},
			ZCrusher = new FloatCrusher(Axis.Z, TRSType.Position, showEnableToggle: true)
			{
				BitsDeterminedBy = BitsDeterminedBy.HalfFloat,
				AccurateCenter = true
			}
		},
		RotCrusher = new ElementCrusher(TRSType.Quaternion, enableTRSTypeSelector: false)
		{
			hideFieldName = true,
			XCrusher = new FloatCrusher(Axis.X, TRSType.Euler, showEnableToggle: true)
			{
				Bits = 12,
				AccurateCenter = true
			},
			YCrusher = new FloatCrusher(Axis.Y, TRSType.Euler, showEnableToggle: true)
			{
				Bits = 12,
				AccurateCenter = true
			},
			ZCrusher = new FloatCrusher(Axis.Z, TRSType.Euler, showEnableToggle: true)
			{
				Bits = 12,
				AccurateCenter = true
			},
			QCrusher = new QuatCrusher(44, showEnableToggle: true, isStandalone: false)
		},
		SclCrusher = new ElementCrusher(TRSType.Scale, enableTRSTypeSelector: false)
		{
			hideFieldName = true,
			uniformAxes = ElementCrusher.UniformAxes.NonUniform,
			XCrusher = new FloatCrusher(BitPresets.Bits8, -1f, 1f, Axis.X, TRSType.Scale, showEnableToggle: true)
			{
				TRSType = TRSType.Scale,
				AccurateCenter = true,
				BitsDeterminedBy = BitsDeterminedBy.SetBits
			},
			YCrusher = new FloatCrusher(BitPresets.Bits8, -1f, 1f, Axis.Y, TRSType.Scale, showEnableToggle: true)
			{
				TRSType = TRSType.Scale,
				AccurateCenter = true,
				BitsDeterminedBy = BitsDeterminedBy.SetBits,
			...
			ZCrusher = new FloatCrusher(BitPresets.Bits8, -1f, 1f, Axis.Z, TRSType.Scale, showEnableToggle: true)
			{
				TRSType = TRSType.Scale,
				AccurateCenter = true,
				BitsDeterminedBy = BitsDeterminedBy.SetBits,
	...
	public override bool AllowInterpolation => allowInterpolation;

	public override bool AllowReconstructionOfEmpty => allowReconstructionOfEmpty;

	public override int ApplyOrder => 9;
	...
	{
		teleNewParentId = (newParent ? newParent.GetInstanceID() : (((state & ObjState.Mounted) != ObjState.Despawned) ? (-2) : (-1)));
	}
	...
	public virtual void OnCaptureCurrentState(int frameId)
	{
		...
		if (hasTeleported)
		{
			...
			CaptureCurrent(frame.telem, frame.telecm, forceUseTransform: true);
