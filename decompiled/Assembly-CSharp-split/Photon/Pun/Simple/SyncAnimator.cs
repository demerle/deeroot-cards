...
public class SyncAnimator : SyncObject<SyncAnimator.Frame>, IOnSnapshot, IOnNetSerialize, IOnAuthorityChanged, ISyncAnimator, IReadyable, IUseKeyframes, IOnInterpolate, IOnCaptureState
{
	...
	public override int ApplyOrder => 11;
	...
	protected override void PopulateFrames()
	{
		...
		if (!masterSharedFramePool.TryGetValue(prefabInstanceId, out var value))
		{
	...
	private void ConnectSharedCaches()
	{
		...
		if (masterSharedParamDefaults.TryGetValue(prefabInstanceId, out var value))
		{
		...
		if (masterSharedParamSettings.TryGetValue(prefabInstanceId, out var value2))
		{
	...
	private void InterpolateParams(float t)
	{
		...
		for (int i = 0; i < paramCount; i++)
		{
			...
			case AnimatorControllerParameterType.Int:
			{
				...
				{
					int value = parameterInterpolation2 switch
					{
						ParameterInterpolation.Lerp => (int)Mathf.Lerp(smartVar, smartVar2, t), 
						ParameterInterpolation.Advance => smartVar2, 
						_ => parameterSettings.defaultValue, 
					};
			...
			case AnimatorControllerParameterType.Float:
			{
				...
				{
					SmartVar smartVar3 = parameterInterpolation switch
					{
						ParameterInterpolation.Advance => smartVar2, 
						ParameterInterpolation.Lerp => Mathf.Lerp(smartVar, smartVar2, t), 
						_ => parameterSettings.defaultValue, 
					};
					animator.SetFloat(hash, smartVar3);
				}
				break;
			}
			case AnimatorControllerParameterType.Bool:
	...
	private void ExtrapolateParams(Frame prev, Frame targ, Frame newtarg)
	{
		...
		for (int i = 0; i < paramCount; i++)
		{
			...
			if (useGlobalParamSettings || parameterSettings.include)
			{
				...
				case AnimatorControllerParameterType.Float:
				{
					ParameterExtrapolation num = (useGlobalParamSettings ? sharedParamDefaults.extrapolateFloats : parameterSettings.extrapolate);
					newtarg.parameters[i] = num switch
					{
						ParameterExtrapolation.Lerp => (float)parameters2[i] + ((float)parameters2[i] - (float)parameters[i]), 
						ParameterExtrapolation.Hold => parameters2[i], 
						_ => parameterSettings.defaultValue, 
					};
					break;
				}
				case AnimatorControllerParameterType.Int:
				{
					ParameterExtrapolation num2 = (useGlobalParamSettings ? sharedParamDefaults.extrapolateInts : parameterSettings.extrapolate);
					newtarg.parameters[i] = num2 switch
					{
						ParameterExtrapolation.Lerp => (int)parameters2[i] + ((int)parameters2[i] - (int)parameters[i]), 
						ParameterExtrapolation.Hold => parameters2[i], 
						_ => parameterSettings.defaultValue, 
					};
					break;
				...
				case AnimatorControllerParameterType.Bool:
				{
					ParameterExtrapolation parameterExtrapolation2 = (useGlobalParamSettings ? sharedParamDefaults.extrapolateBools : parameterSettings.extrapolate);
					newtarg.parameters[i] = ((parameterExtrapolation2 == ParameterExtrapolation.Hold) ? parameters2[i] : parameterSettings.defaultValue);
					break;
				}
				default:
				{
	...
	{
		if (snapFrame != null && snapFrame.content != FrameContents.Empty)
		{
