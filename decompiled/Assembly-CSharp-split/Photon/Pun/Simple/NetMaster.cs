...
public class NetMaster : MonoBehaviour
{
	...
	public static int CurrentFrameId => _currFrameId;

	public static int CurrentSubFrameId => _currSubFrameId;

	public static int PreviousFrameId => _prevFrameId;

	public static int PreviousSubFrameId => _prevSubFrameId;

	public static float NormTimeSinceFixed { get; private set; }

	public static float RTT => rtt;
	...
	private static void SerializeAllAndSend()
	{
		...
		if (flags != SerializationFlags.None)
		{
			...
			reusableBuffer.Send(bitposition, null, flags, flush: true);
		}
	}
	...
	{
		_ = TickEngineSettings.frameCount;
		int bitposition = 0;
		int originFrameId = (int)buffer.Read(ref bitposition, TickEngineSettings.frameCountBits);
		TickManager.LogIncomingFrame(connId, originFrameId, out var arrival);
		while (true)
		{
