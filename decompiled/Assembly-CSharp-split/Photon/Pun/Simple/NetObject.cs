...
public class NetObject : MonoBehaviour, IMatchmakingCallbacks, IOnPhotonViewPreNetDestroy, IPhotonViewCallback, IOnPhotonViewOwnerChange, IOnPhotonViewControllerChange, IOnPreUpdate, IOnPreSimulate, IOnPostSimulate, IOnQuantize, IOnIncrementFrame, IOnPreQuit
{
	...
	public bool SkipWhenEmpty => skipWhenEmpty;

	public bool IgnoreNonControllerUpdates => ignoreNonControllerUpdates;

	public bool ResimulateLateArrivals => resimulateLateArrivals;
	...
	protected void Awake()
	{
		...
		if (!_rigidbody)
		{
			_rigidbody = base.transform.GetNestedComponentInChildren<Rigidbody, NetObject>(includeInactive: true);
		}
		if (!_rigidbody)
		{
			_rigidbody2D = base.transform.GetNestedComponentInChildren<Rigidbody2D, NetObject>(includeInactive: true);
		}
	...
	private void Start()
	{
		...
		{
			OnChangeAuthority(photonView.IsMine, controllerHasChanged: true);
		}
	...
	private void OnDisable()
	{
		...
		NetMasterCallbacks.RegisterCallbackInterfaces(this, register: false);
		DetermineActiveAndControlled(photonView.IsMine);
		int i = 0;
	...
	private void CollectAndReorderInterfaces()
	{
		...
		for (; i <= 24; i++)
		{
			for (int j = 0; j < count; j++)
			{
				...
				if (!(component is IApplyOrder applyOrder))
				{
					if (i == 13)
					{
	...
	{
		AddInterfaces(comp, remove: true);
	}
	...
	private void AddInterfaces(Component comp, bool remove = false)
	{
		...
		AddInterfaceToList(comp, onNetSerializeCallbacks, remove, checkSerializationOptional: true);
		AddInterfaceToList(comp, onQuantizeCallbacks, remove, checkSerializationOptional: true);
		AddInterfaceToList(comp, onIncrementFramesCallbacks, remove, checkSerializationOptional: true);
		AddInterfaceToList(comp, onSnapshotCallbacks, remove, checkSerializationOptional: true);
		AddInterfaceToList(comp, onCriticallyLateFrameCallbacks, remove, checkSerializationOptional: true);
		AddInterfaceToList(comp, onInterpolateCallbacks, remove, checkSerializationOptional: true);
	...
	{
		if (comp is T val && (!checkSerializationOptional || !(val is ISerializationOptional { IncludeInSerialization: false })))
		{
			T item = comp as T;
	...
	private void AddPackObjects(Component comp)
	{
		...
		if (comp.GetType().GetCustomAttributes(typeof(PackObjectAttribute), inherit: false).Length != 0)
		{
	...
	public SerializationFlags OnNetSerialize(int frameId, byte[] buffer, ref int bitposition, SerializationFlags writeFlags)
	{
		if (photonView.Group != 0)
		{
			...
			buffer.WriteBool(b: true, ref bitposition2);
			...
			if (serializationFlags != SerializationFlags.None || !SkipWhenEmpty)
			{
	...
	public bool OnSnapshot(int localTargFrameId)
	{
		...
		if (!TickManager.perConnOffsets.TryGetValue(photonView.ControllerActorNr, out var value))
		{
			return false;
		}
		if (value == null)
	...
	private void BroadcastReadyMaskChange(PackObjRecord p)
	{
		...
		p.onReadyCallback?.OnPackObjReadyChange(p.readyMask, p.readyMask.AllAreTrue);
