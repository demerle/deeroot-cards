...
public static class GenericHitscanExt
{
	...
	private static void CreateDebugPrimitives()
	{
		...
		Pool.AddPrefabToPool(DebugSpherePrefab.gameObject, 2, 2, null, tidyUp: true);
		DebugCubePrefab = GameObject.CreatePrimitive(PrimitiveType.Cube).SetUpDebugPrimitive("DebugCubePrefab");
		Pool.AddPrefabToPool(DebugCubePrefab.gameObject, 2, 2, null, tidyUp: true);
		DebugCylinderPrefab = new GameObject().SetUpDebugPrimitive("DebugCylinderPrefab", createCylinderChild: true);
		Pool.AddPrefabToPool(DebugCylinderPrefab.gameObject, 4, 4, null, tidyUp: true);
	}
	...
	public static int GenericHitscanNonAlloc(this HitscanDefinition hd, Transform origin, NetObject ownerNetObj, ref List<NetworkHit> hitscanHits, ref int nearestIndex, bool showDebugWidgets = false, bool useSecondaryRealm = false)
	{
		...
		if (num > 0)
		{
			for (int i = 0; i < num; i++)
			{
				...
				int num2 = collider.GetComponent<IContactGroupsAssign>()?.Mask ?? 0;
				int value;
	...
	public static int GenericHitscanNonAlloc(this HitscanDefinition hd, Transform origin, ref Collider[] hits, ref RaycastHit[] rayhits, ref int nearestIndex, bool showDebugWidgets = false, bool useSecondaryRealm = false)
	{
		...
		{
			HitscanType.Raycast => Physics.RaycastNonAlloc(origin.position, origin.forward, rayhits, hd.distance, layerMask), 
			HitscanType.SphereCast => Physics.SphereCastNonAlloc(new Ray(vector, origin.forward), hd.radius, rayhits, hd.distance, layerMask), 
			HitscanType.BoxCast => Physics.BoxCastNonAlloc(vector, hd.halfExtents, origin.forward, rayhits, Quaternion.Euler(origin.eulerAngles + hd.orientation), hd.distance, layerMask), 
			HitscanType.CapsuleCast => Physics.CapsuleCastNonAlloc(origin.TransformPoint(hd.offset1), origin.TransformPoint(hd.offset2), hd.radius, origin.forward, rayhits, hd.distance, layerMask), 
			HitscanType.OverlapSphere => Physics.OverlapSphereNonAlloc(vector, hd.radius, hits, layerMask), 
			HitscanType.OverlapBox => Physics.OverlapBoxNonAlloc(vector, hd.halfExtents, hits, Quaternion.Euler(origin.eulerAngles + hd.orientation), layerMask), 
			HitscanType.OverlapCapsule => Physics.OverlapCapsuleNonAlloc(origin.TransformPoint(hd.offset1), origin.TransformPoint(hd.offset2), hd.radius, hits, layerMask), 
			_ => 0, 
		};
		nearestIndex = -1;
		if (num == 0)
	...
	public static int GenericCastNonAlloc(this Transform srcT, Collider[] hits, RaycastHit[] rayhits, float distance, float radius, int mask, Quaternion orientation, bool useOffset, Vector3 offset1, Vector3 offset2, HitscanType hitscanType)
	{
		...
		int num = hitscanType switch
		{
			HitscanType.Raycast => Physics.RaycastNonAlloc(new Ray(vector, srcT.forward), rayhits, distance, mask), 
			HitscanType.SphereCast => Physics.SphereCastNonAlloc(new Ray(vector, srcT.forward), radius, rayhits, distance, mask), 
			HitscanType.BoxCast => Physics.BoxCastNonAlloc(vector, offset2, srcT.forward, rayhits, orientation, distance, mask), 
			HitscanType.CapsuleCast => Physics.CapsuleCastNonAlloc(srcT.TransformPoint(offset1), srcT.TransformPoint(offset2), radius, srcT.forward, rayhits, distance, mask), 
			HitscanType.OverlapSphere => Physics.OverlapSphereNonAlloc(vector, radius, hits, mask), 
			HitscanType.OverlapBox => Physics.OverlapBoxNonAlloc(vector, offset2, hits, orientation, mask), 
			HitscanType.OverlapCapsule => Physics.OverlapCapsuleNonAlloc(srcT.TransformPoint(offset1), srcT.TransformPoint(offset2), radius, hits, mask), 
			_ => 0, 
		};
