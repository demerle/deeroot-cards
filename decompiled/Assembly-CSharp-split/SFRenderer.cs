...
public class SFRenderer : MonoBehaviour
{
	...
	private void RenderLightMap(Matrix4x4 viewMatrix, Matrix4x4 projection, Matrix4x4 vpMatrix, RenderTexture target, List<SFLight> lights, List<SFPolygon> polys, Color ambient, bool hdr)
	{
		...
		GL.Clear(clearDepth: false, clearColor: true, ambient);
		for (int i = 0; i < lights.Count; i++)
		{
			...
			Matrix4x4 matrix4x = sFLight._ModelMatrix(forceProjection: false) * sFLight._CookieMatrix();
			...
			if (polys != null && (int)sFLight._shadowLayers != 0)
			{
				...
				if (mesh != null)
				{
					...
					Graphics.DrawMeshNow(mesh, sFLight._ModelMatrix(forceProjection: true));
					mesh.Clear();
				}
				if (hdr)
				{
	...
	private static Rect CullLights(Matrix4x4 vpMatrix, List<SFLight> lights, List<SFLight> culledLights)
	{
		...
		{
			SFLight sFLight = lights[i];
			if (_FastCull(vpMatrix * sFLight._ModelMatrix(forceProjection: false), sFLight._bounds))
			{
	...
	private void OnPreRender()
	{
		...
		{
			lights = new List<SFLight>(from o in UnityEngine.Object.FindObjectsOfType<SFLight>()
				where o.isActiveAndEnabled
				select o);
			list = new List<SFPolygon>(from o in UnityEngine.Object.FindObjectsOfType<SFPolygon>()
				where o.isActiveAndEnabled
				select o);
